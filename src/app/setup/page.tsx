...
export default function Setup() {
  ...
  const saveBasics = async () => {
    ...
    setSaving(true);
    setError('');
    const chosen = (slug || suggested).toLowerCase().replace(/[^a-z0-9-]/g, '');
    ...
    setSaving(false);
    if (e) {
      setError(e.message.includes('duplicate') ? 'That address is taken, try another' : e.message);
      return;
    }
    setSlug(chosen);
    setStep(2);
  ...
  const pageUrl = 'openstatus.co/' + (slug || suggested);
      ...
      <div className="max-w-lg mx-auto px-6 py-10">
        <div className="flex items-center gap-1.5">
          {[1, 2, 3, 4, 5].map((n) => (
            <span
              key={n}
              className={'h-1 flex-1 rounded-full ' + (n <= step ? 'bg-[#2E7D5B]' : 'bg-[#E8E6E0]')}
            />
          ))}
        </div>
        ...
        {step === 1 ? (
          <div className="mt-6">
            ...
            <p className="mt-2 text-[#6C6A62]">This is what customers see at the top of your page.</p>
            ...
            <div className="mt-7 space-y-3">
              ...
              <input
                ...
                placeholder="Coffee shop · Nashville, TN"
                className="w-full px-4 py-3 rounded-2xl bg-[#F5F7F5] placeholder-[#9B998F] focus:outline-none focus:ring-2 focus:ring-[#2E7D5B]/30"
              />
              <div className="rounded-2xl bg-[#F5F7F5] px-4 py-3">
                <p className="text-xs text-[#6C6A62]">Your page address</p>
                <div className="flex items-center gap-1 mt-1">
                  <span className="text-[#9B998F]">openstatus.co/</span>
                  <input
                    value={slug}
                    onChange={(e) => setSlug(e.target.value)}
                    placeholder={suggested || 'yourbusiness'}
        ...
        {step === 2 ? (
          <div className="mt-6">
            ...
            <p className="mt-2 text-[#6C6A62]">
              Set these once. We watch for changes and update your page whenever
              something is different, then put it back to normal on its own.
            </p>
              ...
              {DAYS.map((d, i) => (
                <div key={d} className="flex items-center gap-3 rounded-2xl bg-[#F5F7F5] px-4 py-2.5">
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1.5 rounded-full bg-white text-[#6C6A62] hover:text-[#1A1A18] transition"
                  >
        ...
        {step === 4 ? (
          <div className="mt-6">
            ...
            <p className="mt-2 text-[#6C6A62]">
              Post like you always do. We read your posts for anything customers need
              to know today, and update your page on its own.
            </p>
            ...
            <div className="mt-6 rounded-2xl bg-[#F5F7F5] px-5 py-4">
              <p className="text-sm text-[#4A4842]">
                ...
                read posts, we never post anything.
              </p>
            </div>
        ...
        {step === 5 ? (
          <div className="mt-6">
            <h1 className="text-3xl font-bold tracking-tight">Your page is live</h1>
            ...
            <div className="mt-6 rounded-2xl bg-[#12251D] text-white px-5 py-5">
              ...
              <p className="mt-2 text-lg font-medium">Before you go</p>
              <p className="text-[#9FE1CB]">{pageUrl}</p>
              <button
                onClick={() => {
                  navigator.clipboard.writeText('Before you go\n' + pageUrl);
                  setCopied(true);
            ...
            <a
              href={'/' + (slug || suggested)}
              className="mt-4 block text-center py-3.5 rounded-full border border-black/15 font-medium hover:border-black/50 transition"
            >
              View my page
            </a>
            <a
              href="/dashboard"
              className="mt-3 block text-center py-3.5 rounded-full bg-[#1A1A18] text-white font-medium hover:bg-[#2E7D5B] transition"
            >
              Go to dashboard
            </a>
          </div>
