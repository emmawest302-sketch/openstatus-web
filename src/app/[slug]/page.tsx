...
function Icon({ name, size = 22 }: { name: string; size?: number }) {
  const p: Record<string,string> = {
    ...
    cup: 'M4 8h13v5a5 5 0 01-5 5H9a5 5 0 01-5-5V8zM17 9h2a2 2 0 010 4h-2M7 4c0 1 .8 1 .8 2M11 4c0 1 .8 1 .8 2',
...
function iconFor(label: string): string {
  ...
  if (s.includes('reserv') || s.includes('table')) return 'calendar';
...
export default async function LiveStatus({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const glass = 'backdrop-blur-xl bg-white/70 border border-white/60';
  ...
  return (
    <div className="min-h-screen bg-[#EDE9E2] text-[#1A1A18] flex justify-center" style={{ fontFamily: 'var(--font-display)' }}>
      <div className="relative w-full max-w-[440px] min-h-screen overflow-hidden">
        ...
        <div className="absolute inset-x-0 top-0 h-[420px] bg-gradient-to-b from-black/25 via-black/10 to-[#EDE9E2]" />

        <div className="relative px-4 pb-10">
          <div className="pt-32 flex items-end gap-4">
            {business.avatar_url ? (
            ...
            <div className="pb-1 min-w-0">
              <h1 className="text-[27px] font-bold tracking-[-0.02em] text-white leading-tight drop-shadow-sm truncate">
                {business.name}
              </h1>
              {business.tagline ? (
                <p className="text-sm text-white/90 drop-shadow-sm truncate">{business.tagline}</p>
              ) : null}
            </div>
          </div>

          <div className={'mt-5 rounded-[26px] p-4 ' + glass}>
            ...
            {changed && lead ? (
              ...
              <div className="mt-4 flex items-center gap-3 rounded-[18px] bg-white/55 px-4 py-3.5">
                <span className="text-[#2E7D5B] shrink-0"><Icon name="shield" /></span>
                <div className="flex-1 min-w-0">
                  <p className="font-medium">No changes detected today</p>
                  <p className="text-sm text-[#6C6A62]">
                    {business.instagram_handle ? 'Instagram checked a few minutes ago' : 'Regular hours apply'}
                  </p>
                </div>
              </div>
            )}
          ...
          <details className={'mt-3 rounded-[26px] overflow-hidden ' + glass}>
          ...
          {links.length > 0 ? (
            <div className="mt-3 grid grid-cols-3 gap-2.5">
              {links.map((l) => (
                <a key={l.url} href={l.url} target="_blank" rel="noreferrer" className={'rounded-[20px] px-3 py-4 hover:bg-white/85 transition ' + glass}>
          ...
          <div className={'mt-3 rounded-[26px] px-5 py-4 flex items-center gap-4 ' + glass}>
            <span className="w-12 h-12 rounded-full bg-[#E2EFE7] flex items-center justify-center text-[#2E7D5B] shrink-0">
              <Icon name="bell" />
            </span>
            <div className="flex-1 min-w-0">
              <p className="font-medium">Get important updates</p>
              <p className="text-sm text-[#6C6A62] leading-snug">
                Text alerts for closures and special hours.
              </p>
            </div>
          </div>

          {business.instagram_handle ? (
            <p className="mt-5 text-center text-sm text-[#6C6A62]">
              Follow us{' '}
