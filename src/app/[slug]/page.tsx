...
export default async function LiveStatus({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: business } = await admin
    .from('businesses')
    .select('id, name, tagline, avatar_url, links, instagram_handle')
    ...
    <div className="min-h-screen bg-[#F6F4EF] text-[#1A1A18] flex justify-center sm:py-8" style={{ fontFamily: 'var(--font-display)' }}>
      <div className="w-full max-w-[440px] px-4 pb-10">
        <div className="pt-8 pb-6 text-center">
          ...
          {business.tagline ? (
            <p className="mt-1 text-xs tracking-[0.16em] uppercase text-[#7C7A72]">{business.tagline}</p>
          ) : null}
        </div>
        ...
        <div className="rounded-[22px] bg-white p-5">
          ...
          <p className="mt-1.5 text-[#5C5A52]">{sub}</p>
        ...
        {links.length > 0 ? (
          <>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2.5">
              {links.map((l) => (
                <a key={l.url} href={l.url} target="_blank" rel="noreferrer" className="rounded-[18px] bg-white px-4 py-4 hover:bg-[#FBFAF7] transition">
        ...
        {business.instagram_handle ? (
          <p className="mt-7 text-center text-sm text-[#7C7A72]">
            Follow us{' '}
            <a href={'https://instagram.com/' + business.instagram_handle} target="_blank" rel="noreferrer" className="text-[#0F6E56]">
              @{business.instagram_handle}
            </a>
          </p>
        ) : null}

        <a href="/" className="mt-4 block text-center text-[10px] uppercase tracking-[0.18em] text-[#A3A199] hover:text-[#1A1A18] transition" style={{ fontFamily: 'var(--font-mono)' }}>
