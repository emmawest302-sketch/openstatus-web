...
type PublicActionBlockProps = { block: OpenStatusBlock };
...
export default function PublicActionBlock({ block }: PublicActionBlockProps) {
  ...
  return href ? <a href={href} target="_blank" rel="noreferrer" className={classes} aria-label={`${block.title}: ${block.sub}`}>{content}</a> : <div className={classes}>{content}</div>;
}
