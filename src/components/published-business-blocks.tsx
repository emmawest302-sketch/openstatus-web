...
export default function PublishedBusinessBlocks({ businessName, location, config }: Props) {
  ...
  return (
    <section className="grid grid-cols-2 gap-2.5 px-4 py-3">
      {enabled.map((block) =>
        block.id === 'map' ? (
          <PublicMapBlock key={block.id} name={businessName} location={location} />
        ) : (
          <PublicActionBlock key={block.id} block={block} />
        ),
      )}
    </section>
  );
