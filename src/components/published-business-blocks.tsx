...
export default function PublishedBusinessBlocks({ businessId, businessName, location, config }: Props) {
  ...
  return <section className="grid grid-cols-2 gap-2.5 px-4 py-3">{enabled.map((block) => block.id === 'map' ? <PublicMapBlock key={block.id} businessId={businessId} name={businessName} location={block.url?.trim() || location} /> : <PublicActionBlock key={block.id} businessId={businessId} block={block} />)}</section>;
}
