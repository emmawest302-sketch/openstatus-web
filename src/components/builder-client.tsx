...
export default function BuilderClient(){
 ...
 useEffect(()=>{void(async()=>{
  ...
  setBlocks(config.blocks);setBg(config.bg);setSocials(config.socials);setLocation(config.location??'');setTags(config.tags??[]);
 ...
 const save=async()=>{setSaving(true);setError('');const config:OpenStatusPageConfig={blocks,bg,socials,location,tags};const{error:e}=await supabase.auth.updateUser({data:{openstatus_page:config}});setSaving(false);if(e){setError(e.message);return}setSaved(true)};
 ...
 return <main className="min-h-screen bg-[#F5F3ED] text-[#101010]" style={{fontFamily:'var(--font-poppins)'}}>
  ...
  <div className="mx-auto grid w-[min(96%,1440px)] gap-5 py-5 lg:grid-cols-[240px_minmax(420px,1fr)_420px]">
   <aside className="rounded-[28px] border border-black/8 bg-white/65 p-4"><span className="text-[9px] font-bold tracking-[.14em] text-black/35">PAGE BUILDER</span><nav className="mt-4 space-y-1"><button className="flex w-full items-center justify-between rounded-[16px] bg-black px-3 py-3 text-left text-sm font-semibold text-white"><span>Page</span><span className="opacity-40">›</span></button><Link href="/dashboard" className="flex w-full items-center justify-between rounded-[16px] px-3 py-3 text-left text-sm font-semibold hover:bg-black/5"><span>Status</span><span className="opacity-40">›</span></Link><div className="flex w-full items-center justify-between rounded-[16px] px-3 py-3 text-left text-sm font-semibold"><span>Socials</span><span className="opacity-40">↓</span></div><Link href="/analytics" className="flex w-full items-center justify-between rounded-[16px] px-3 py-3 text-left text-sm font-semibold hover:bg-black/5"><span>Analytics</span><span className="opacity-40">›</span></Link><Link href="/settings" className="flex w-full items-center justify-between rounded-[16px] px-3 py-3 text-left text-sm font-semibold hover:bg-black/5"><span>Settings</span><span className="opacity-40">›</span></Link></nav><div className="mt-8 rounded-[20px] bg-[#CBD9FF]/45 p-4"><strong className="text-sm">Your link</strong><span className="mt-1 block break-all text-xs text-black/45">openstatus.co/{business?.slug||'yourbusiness'}</span></div></aside>
   ...
   <section className="rounded-[30px] border border-black/8 bg-white/55 p-5 md:p-7"><div className="flex items-end justify-between gap-4"><div><span className="text-[9px] font-bold tracking-[.14em] text-black/35">{business?.name?.toUpperCase()}</span><h1 className="mt-2 text-4xl font-semibold tracking-[-.055em]">Build the front door.</h1><p className="mt-2 text-xs text-black/40">Everything here publishes directly to your customer-facing page.</p></div><button onClick={()=>setShowLibrary(true)} className="shrink-0 rounded-full border border-black/10 bg-white px-4 py-2.5 text-xs font-bold shadow-sm">＋ Add block</button></div>
    {business?<div className="mt-7"><BusinessBrandAssets business={business} onChange={next=>{setBusiness(next as Business);dirty()}}/></div>:null}
    <div className="mt-4 rounded-[24px] bg-[#F5F3ED] p-4"><div className="flex items-center justify-between"><div><strong className="text-sm">Page color</strong><span className="mt-1 block text-xs text-black/40">Shows behind your blocks below the background image</span></div><div className="flex gap-1">{['warm','blue','lime','dark'].map(x=><button key={x} onClick={()=>{setBg(x);dirty()}} className={`h-8 w-8 rounded-full border-2 ${bg===x?'border-black':'border-white'} ${x==='warm'?'bg-[#E8D5BF]':x==='blue'?'bg-[#CBD9FF]':x==='lime'?'bg-[#C8FF62]':'bg-[#222]'}`} aria-label={x}/>)}</div></div></div>
   ...
   <aside className="rounded-[30px] bg-[#E8E4DA] p-5"><div className="mb-3 flex justify-between text-[9px] font-bold tracking-[.13em] text-black/35"><span>LIVE PREVIEW</span><span>Mobile</span></div><div className={`mx-auto max-w-[360px] overflow-hidden rounded-[42px] border-[8px] border-black shadow-[0_28px_70px_rgba(0,0,0,.22)] ${bg==='warm'?'bg-[#E8D5BF]':bg==='blue'?'bg-[#CBD9FF]':bg==='lime'?'bg-[#DFFFA8]':'bg-[#333] text-white'}`}><div className="relative h-52 bg-gradient-to-br from-[#D1A171] via-[#8B6B4E] to-[#46533C]">{cover?<img src={cover} alt="" className="absolute inset-0 h-full w-full object-cover"/>:null}<div className="absolute inset-0 bg-black/30"/><div className="absolute inset-x-5 bottom-5 text-white">{logo?<img src={logo} alt="" className="mb-3 h-11 w-11 rounded-full bg-white object-cover"/>:<div className="mb-3 grid h-11 w-11 place-items-center rounded-full bg-white text-[10px] font-extrabold text-[#263829]">{business?.name?.slice(0,2).toUpperCase()}</div>}<strong className="block text-2xl tracking-[-.05em]">{business?.name}</strong><span className="text-[11px] text-white/70">{business?.tagline||'Your business, right now.'}</span></div></div><div className="space-y-2 p-3"><div className="rounded-[18px] bg-white/90 p-3 text-black"><span className="text-[8px] font-bold tracking-[.12em] text-[#2E7D5B]">● LIVE STATUS</span><strong className="mt-1 block text-xl tracking-[-.04em]">Open now</strong><span className="text-[10px] text-black/45">Closes at 4:00 PM</span></div>{blocks.filter(b=>b.on).map(b=><div key={b.id} className={`${b.tone==='dark'?'bg-black text-white':'bg-white/80 text-black'} rounded-[18px] p-3 backdrop-blur`}><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-black/5">{b.icon}</span><div><strong className="block text-xs">{b.title}</strong><span className="text-[9px] opacity-45">{b.sub}</span></div></div></div>)}{socials.some(s=>s.on&&s.url)&&<div className="flex justify-center gap-2 py-2">{socials.filter(s=>s.on&&s.url).map(s=><span key={s.id} className="grid h-8 w-8 place-items-center rounded-full bg-white/80 text-[9px] font-bold text-black">{socialMark(s.label)}</span>)}</div>}<div className="py-2 text-center text-[8px] font-semibold tracking-[.13em] opacity-30">POWERED BY OPENSTATUS</div></div></div></aside>
  </div>
  {showLibrary&&<div className="fixed inset-0 z-50 grid place-items-center bg-black/35 p-4 backdrop-blur-sm" onMouseDown={()=>setShowLibrary(false)}><div className="w-full max-w-xl rounded-[30px] bg-[#F8F7F2] p-6 shadow-2xl" onMouseDown={e=>e.stopPropagation()}><div className="flex items-start justify-between"><div><span className="text-[9px] font-bold tracking-[.14em] text-black/35">BLOCK LIBRARY</span><h2 className="mt-2 text-3xl font-semibold tracking-[-.05em]">What do customers need?</h2></div><button onClick={()=>setShowLibrary(false)} className="grid h-9 w-9 place-items-center rounded-full bg-black/5">×</button></div><div className="mt-5 grid gap-2 sm:grid-cols-2">{BLOCK_LIBRARY.map(t=><button key={t.id} onClick={()=>addBlock(t)} className="rounded-[18px] border border-black/8 bg-white p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md"><span className="text-lg">{t.icon}</span><strong className="mt-2 block text-sm">{t.title}</strong><span className="text-[10px] text-black/40">{t.sub}</span></button>)}</div></div></div>}
 </main>;
}
