...
type Block={id:string;title:string;sub:string;icon:string;on:boolean;tone:string};
type Business={id:string;name:string;tagline:string|null;slug:string|null;avatar_url:string|null;header_url:string|null};
type Config={blocks:Block[];bg:string;socials:string[]};
...
export default function BuilderClient(){
 ...
 const[socials,setSocials]=useState<string[]>(['Instagram','Facebook']);
 ...
 useEffect(()=>{void(async()=>{
  ...
  const meta=userData.user.user_metadata?.openstatus_page as Config|undefined;
  if(meta){if(Array.isArray(meta.blocks))setBlocks(meta.blocks);if(typeof meta.bg==='string')setBg(meta.bg);if(Array.isArray(meta.socials))setSocials(meta.socials)}
 ...
 const move=(i:number,d:number)=>{const j=i+d;if(j<0||j>=blocks.length)return;const n=[...blocks];[n[i],n[j]]=[n[j],n[i]];setBlocks(n);setSaved(false)};
 const toggle=(id:string)=>{setBlocks(v=>v.map(b=>b.id===id?{...b,on:!b.on}:b));setSaved(false)};
 const addSocial=()=>{const next=['TikTok','YouTube','LinkedIn','Pinterest'].find(s=>!socials.includes(s));if(next){setSocials([...socials,next]);setSaved(false)}};
 const save=async()=>{setSaving(true);setError('');const config:Config={blocks,bg,socials};const{error:e}=await supabase.auth.updateUser({data:{openstatus_page:config}});setSaving(false);if(e){setError(e.message);return}setSaved(true)};
 ...
 return <main className="min-h-screen bg-[#F5F3ED] text-[#101010]" style={{fontFamily:'var(--font-poppins)'}}>
  ...
  <div className="mx-auto grid w-[min(96%,1440px)] gap-5 py-5 lg:grid-cols-[240px_minmax(420px,1fr)_420px]">
   ...
   <section className="rounded-[30px] border border-black/8 bg-white/55 p-5 md:p-7"><div className="flex items-end justify-between"><div><span className="text-[9px] font-bold tracking-[.14em] text-black/35">{business?.name?.toUpperCase()}</span><h1 className="mt-2 text-4xl font-semibold tracking-[-.055em]">Build the front door.</h1></div><button className="rounded-full border border-black/10 bg-white px-4 py-2 text-xs font-semibold">＋ Add block</button></div>
    <div className="mt-7 rounded-[24px] bg-[#F5F3ED] p-4"><div className="flex items-center justify-between"><div><strong className="text-sm">Background</strong><span className="mt-1 block text-xs text-black/40">Photo, color or soft gradient</span></div><div className="flex gap-1">{['warm','blue','lime','dark'].map(x=><button key={x} onClick={()=>{setBg(x);setSaved(false)}} className={`h-8 w-8 rounded-full border-2 ${bg===x?'border-black':'border-white'} ${x==='warm'?'bg-[#E8D5BF]':x==='blue'?'bg-[#CBD9FF]':x==='lime'?'bg-[#C8FF62]':'bg-[#222]'}`} aria-label={x}/>)}</div></div></div>
    <div className="mt-4 space-y-2">{blocks.map((b,i)=><div key={b.id} className={`flex items-center gap-3 rounded-[20px] border border-black/8 bg-white p-3 ${!b.on?'opacity-45':''}`}><span className="text-black/25">⋮⋮</span><div className={`grid h-12 w-12 place-items-center rounded-[14px] ${b.tone==='dark'?'bg-black text-white':b.tone==='map'?'bg-[#DCE5D0]':'bg-[#F2EFE8]'}`}>{b.icon}</div><div className="min-w-0 flex-1"><strong className="block text-sm">{b.title}</strong><span className="block truncate text-xs text-black/40">{b.sub}</span></div><div className="flex gap-1"><button onClick={()=>move(i,-1)} className="h-8 w-8 rounded-full bg-black/5 text-xs">↑</button><button onClick={()=>move(i,1)} className="h-8 w-8 rounded-full bg-black/5 text-xs">↓</button><button onClick={()=>toggle(b.id)} className={`h-8 rounded-full px-3 text-[9px] font-bold ${b.on?'bg-[#C8FF62]/65':'bg-black/5'}`}>{b.on?'ON':'OFF'}</button></div></div>)}</div>
    <div className="mt-7 border-t border-black/8 pt-6"><div className="flex items-center justify-between"><div><strong className="text-sm">Social icons</strong><span className="mt-1 block text-xs text-black/40">Manual links now; deeper Meta connection stays separate.</span></div><button onClick={addSocial} className="rounded-full bg-black/5 px-3 py-2 text-xs font-bold">＋ Add</button></div><div className="mt-3 flex flex-wrap gap-2">{socials.map(s=><span key={s} className="rounded-full border border-black/10 bg-white px-3 py-2 text-xs font-semibold">{s}</span>)}</div></div>
