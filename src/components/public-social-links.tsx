...
export default function PublicSocialLinks({ socials }:{ socials: Social[] }) {
  const visible=socials.filter(s=>s.on!==false&&s.url?.trim());
  if(!visible.length)return null;
  return <div className="flex flex-wrap justify-center gap-2 px-4 py-4">{visible.map((social,i)=>{const url=href(social.url||'');const key=social.label.toLowerCase();return <a key={social.id||`${key}-${i}`} href={url} target="_blank" rel="noreferrer" aria-label={social.label} className="grid h-11 min-w-11 place-items-center rounded-full border border-black/10 bg-white/70 px-3 text-[11px] font-bold text-black shadow-sm backdrop-blur-xl transition hover:-translate-y-0.5"><span>{marks[key]||social.label.slice(0,2)}</span></a>})}</div>;
}
